  ...
  return (
    <div className='className="fixed top-0 right-0 left-0 mx-auto z-[100] py-0"'>
      <div className="flex" id="navbar-default">
        ...
        {!user?.email ? <ul className="flex gap-4 items-center text-black p-3">
        ...
        </ul> : <ul className="flex gap-4 items-center text-black p-3">
          ...
          <NavLink to={'/Account'} className="bg-[#fe39a2] hover:bg-[#ff51ae] border border-[#fe39a2] py-2 px-3 duration-300 text-white rounded-md flex gap-2 items-center">
            <p className="block">{userInfo?.displayName}</p>
            {userInfo?.userimage === '' || userInfo?.userimage === null || userInfo?.userimage === undefined ? null : <img src={userInfo?.userimage} alt={''} className='h-[28px] w-[28px] rounded-full' />}
